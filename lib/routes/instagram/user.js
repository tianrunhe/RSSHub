...
export default async function handler(ctx) {
    ...
    ctx.state.data = {
        ...
        item: list.map((item, index) => {
            ...
            let content = '';
            const mediaItems = media[index];

            for (const mediaItem of mediaItems) {
                if (mediaItem.image && !mediaItem.video) {
                    content += `<img src="${mediaItem.image}"><br>`;
                    image++;
                }

                if (mediaItem.video) {
                    content += `<video width="100%" controls="controls" poster="${mediaItem.image}"> <source src="${mediaItem.video}" type="video/mp4"> Your RSS reader does not support video playback. </video>`;
                    video++;
                }
            }
